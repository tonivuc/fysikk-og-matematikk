...
list = [Orbit([0.0, 0.0,  (12756.28/2) * 1000, 0, 0.0])]
...
def main():
    ...
    while(True):
        ...
        saturnV2 = SaturnV(m1,c1,d1,ts1,tv1,m2,c2,d2,ts2,tv2,m3,c3,d3,ts3,tv3)
        print("angle1: ", higher_a)
        lower_t = rungeKuttaRun(higher_a)
        print("moh1: " , planetz[0].moh())
        planetz[0] = Orbit([0.0, 0.0,  (12756.28/2) * 1000, 0, 0.0])
        saturnV2 = SaturnV(m1,c1,d1,ts1,tv1,m2,c2,d2,ts2,tv2,m3,c3,d3,ts3,tv3)
        print("angle2: ", lower_a)
        higher_t = rungeKuttaRun(lower_a)
        print("moh2: " , planetz[0].moh())

        planetz[0] = Orbit([0.0, 0.0,  (12756.28/2) * 1000, 0, 0.0])
        saturnV2 = SaturnV(m1,c1,d1,ts1,tv1,m2,c2,d2,ts2,tv2,m3,c3,d3,ts3,tv3)
        midpoint = (higher_a + lower_a)/2
        print("angle3: ", midpoint)
        midpoint_t = rungeKuttaRun(midpoint)
        print("moh3: " , planetz[0].moh())

        print("Mid T: ", midpoint_t)
        ...
        if(lower_t >= goal - tol and lower_t <= goal + tol):
            finalAngle =  higher_a
            break
        elif(higher_t >= goal - tol and higher_t <= goal + tol):
            finalAngle = lower_a
            break
